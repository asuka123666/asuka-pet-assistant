...
const LONG_PRESS_FAILED_MS = 1500;
...
const animations = {
  ...
  waving: { row: 3, frames: 4, fps: 8, holdLastMs: 140 },
  jumping: { row: 4, frames: 5, fps: 7, holdLastMs: 120 },
  failed: { row: 5, frames: 8, fps: 6, holdLastMs: 180 },
  waiting: { row: 6, frames: 6, fps: 4, holdLastMs: 180 },
  running: { row: 7, frames: 6, fps: 9 },
  review: { row: 8, frames: 6, fps: 4.5, holdLastMs: 180 },
  grabbedCollar: { row: 9, frames: 6, fps: 7, holdLastMs: 40, fallback: "waiting" },
  struggleHang: { row: 10, frames: 8, fps: 6.5, fallback: "waiting" },
  dropRecover: { row: 11, frames: 6, fps: 7, holdLastMs: 80, fallback: "failed" },
  lazySit: { row: 12, frames: 6, fps: 5, holdLastMs: 180, fallback: "lazyIdle" },
  doze: { row: 13, frames: 6, fps: 2.5, holdLastMs: 180, fallback: "dozeIdle" },
  pokeAnnoyed: { row: 14, frames: 4, fps: 6.25, holdLastMs: 140, fallback: "failed" },
  stretch: { row: 15, frames: 6, fps: 5.5, holdLastMs: 160, fallback: "stretchIdle" }
};
...
function hasAnimationRow(rowIndex) {
  return Number.isFinite(rowIndex) && rowIndex >= 0 && rowIndex < availableSpriteRows;
}
...
function setFrame(row, column) {
  const x = Math.round(column * CELL_WIDTH * petScale);
  const y = Math.round(row * CELL_HEIGHT * petScale);
...
function canStart(nextPriority) {
  // jumping/failed only interrupted by drag
  if ((active.name === "jumping" || active.name === "failed") && nextPriority !== priority.drag) {
    return false;
  }
  ...
  // recovery state blocks everything except drag
  if (active.name === "recovery" && nextPriority !== priority.drag) return false;
...
function neutralFrame() {
  if (TRANSITION_RESET_TO_NEUTRAL) setFrame(animations.idle.row, 0);
}
...
function playOnce(name, nextPriority, onDone = null, options = {}) {
  ...
  actionEndTimer = setTimeout(() => {
    ...
    if (options.anticipationFrame) {
      setFrame(options.anticipationFrame.row, options.anticipationFrame.frame);
    }
...
function startHeldDrag() {
  ...
  dragMaxDistance = 0;
  maybeShowDragSpeech();
  if (canUseAnimation("grabbedCollar") && canUseAnimation("struggleHang")) {
    resetSpriteTransform();
    playAnimationFramesForActive("grabbedCollar", "dragging", () => {
      if (active.name === "dragging") {
        playAnimationLoopForActive("struggleHang", "dragging");
        startHangSway();
      }
    });
    return true;
...
function updateDragStruggle(distance = 0) {
  ...
  if (canUseAnimation("struggleHang")) {
    resetSpriteTransform();
    return;
  }
...
function playLongPressHeld() {
  if (!startAction("longPressHeld", priority.longPress)) return false;
  longPressStartedAt = Date.now();
  longPressHeld = true;
  if (!chatBubbleOpen) showSpeech("longPress");

  function tick() {
    if (active.name !== "longPressHeld") return;
    const phase = Math.floor(Date.now() / 180) % 3;
    const transforms = [
      { x: -1, y: 1, rotate: -1.5 },
      { x: 1, y: 0, rotate: 1.5 },
      { x: 0, y: 1, rotate: 0 }
    ];
    setFrame(animations.waiting.row, phase);
    setSpriteTransform(transforms[phase]);
    animationTimer = setTimeout(tick, 180);
  }
...
function finishLongPressHeld() {
  const heldDuration = Date.now() - longPressStartedAt;
  longPressHeld = false;
  resetSpriteTransform();
  blockIdleUntil = Date.now() + POST_ACTION_IDLE_DELAY_MS;
  active = { name: "idle", priority: priority.idle };

  if (heldDuration > LONG_PRESS_FAILED_MS) {
    playFailed(priority.longPress);
    return;
  }
...
pet.addEventListener("click", () => {
  if (suppressNextClick || didDrag || longPressPlayed || Date.now() < suppressClickUntil) {
